...
__authors__ = "Aldo Vargas, Lachlan,"
...
class MultiWii:
    ...
    def __init__(self, ser_port):
        ...
        self.rawIMU = {'ax': 0, 'ay': 0, 'az': 0, 'gx': 0, 'gy': 0, 'gz': 0, 'mx': 0, 'my': 0, 'mz': 0, 'elapsed': 0,
                       'timestamp': 0}
        ...
        self.analog = {'vbat': 0, 'powermetersum': 0, 'rssi': 0, 'amperage': 0, 'elapsed': 0, 'timestamp': 0}
        self.message = {'angx': 0, 'angy': 0, 'heading': 0, 'roll': 0, 'pitch': 0, 'yaw': 0, 'throttle': 0,
                        'elapsed': 0, 'timestamp': 0}
        self.misc = {'powertrigger': 0, 'minthrottle': 0, 'maxthrottle': 0, 'mincommand': 0, 'failsafethrottle': 0,
                     'arm': 0, 'lifetime': 0, 'mag_declination': 0, 'vbatscale': 0, 'vbatscale_warn1': 0,
                     'vbatscale_warn2': 0, 'vbatscale_crit': 0, 'elapsed': 0, 'timestamp': 0}
        self.temp = ()
        self.temp2 = ()
        self.elapsed = 0
        self.PRINT = 1
    ...
    def getData(self, cmd):
        try:
            ...
            if cmd == MultiWii.ATTITUDE:
                ...
                self.analog['powermetersum'] = float(temp[1])
                ...
                self.misc['powertrigger'] = float(temp[0])
                self.misc['minthrottle'] = float(temp[1])
                self.misc['maxthrottle'] = float(temp[2])
                self.misc['mincommand'] = float(temp[3])
                self.misc['failsafethrottle'] = float(temp[4])
                self.misc['arm'] = float(temp[5])
                self.misc['lifetime'] = float(temp[6])
                self.misc['mag_declination'] = float(temp[7])
                self.misc['vbatscale'] = float(temp[8])
                self.misc['vbatscale_warn1'] = float(temp[9])
                self.misc['vbatscale_warn2'] = float(temp[10])
                self.misc['vbatscale_crit'] = float(temp[11])
                self.misc['elapsed'] = float(temp[12])
                self.misc['timestamp'] = float(temp[13])
                self.misc['elapsed'] = "%0.3f" % (elapsed,)
                self.misc['timestamp'] = "%0.2f" % (time.time(),)
                return self.misc
    ...
    """
    Function to ask for 2 fixed cmds, attitude and rc channels, and receive them. 
